...
<template>
  <div style="padding:20px;height:100%">
    <div
      class = "tableBox"
      style = "height:95%;background:#fff;padding:5px;"
    >
      <div
        el-row
      ...
      >
        ...
        >
          ...
          <el-form-item label="负责范围">
            ...
            <!-- <div style="margin-left:80px;" id="addAreaButton">
              <el-button type="primary" size="mini" @click="reportItems">打印items</el-button>
            </div>-->
          </el-form-item>
      ...
      <!-- 详情模态框 -->
      <el-dialog title="申请机构详情" :visible.sync="dialogVisible2" width="900px" top="50px" center>
        <el-form
          :model="detailForm"
          label-width="150px"
          class="demo-ruleForm"
    ...
    getData() {
      this.$axios({
        ...
        url: `${this.$store.state.globalRouter}/auth/organization/${
          this.module
        }/pagination?orgType=1&orgCodeLike=${
 ...
 if (res.data.resultList) {
    ...
    dataList(tableData) {
      ...
      datas.forEach((item, index) => {
        ...
        item.orgNature = item.orgNature == 1 ? "公立" : "民营";
      });
      this.lists = datas;
      // console.log(datas);
    },
    ...
    updateData() {
      if (this.checkData.length != 1) {
        ...
        this.ruleForm.legalRepresentativePhone = parseInt(data.legalRepresentativePhone);
        ...
        this.$axios({
          ...
          if (res.data.resultCode == "000000") {
            ...
            // this.ruleForm.manageArea = result.overallType;
    ...
    protocolDetails(index, rowData) {
      ...
      // this.detailForm.orgNature=(data.orgNature=="公立"?1:0);
      // this.detailForm.status=(data.orgNature=="启用"?1:0);
      if (data.orgNature == "公立") {
        this.detailForm.orgNature = 1;
      } else {
        this.detailForm.orgNature = 0;
      }
      if (data.status == "启用") {
      ...
      if (data.bankAccountArea && !Number.isInteger(data.bankAccountArea)) {
        this.getBankAreaChinese(data.bankAccountArea.split("-")[1]);
      } else {
        this.bankProvince = "";
        this.getBankAreaChinese(data.bankAccountArea);
      }
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          areaData.forEach((item, index) => {
            ...
            item.regionStreet = item.regionStreet + "-" + item.street;
          });
          if (this.modelType) {
            // console.log("新增");
            data.orgId = "";
            ...
            this.$axios({
              ...
              if (res.data.data) {
                ...
                //保存负责区域
                // areaData.relationObjectId = res.data.data + "";
                areaData.forEach((item, index) => {
      ...
      for (let i = 0; i < menu.length; i++) {
        if (menu[i].name == "机构管理") {
          ...
          for (let j = 0; j < submenu.length; j++) {
            if (submenu[j].component == "core/apply_org") {
              // console.log("申请机构", submenu[j]);
              // urlRole1==dataAdd 增
