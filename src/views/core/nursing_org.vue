    ...
    <div class="tableBox" style="height:95%;background:#fff;padding:5px;">
      ...
      >
        ...
        <el-form-item label="组织机构代码">
          <el-input
            v-model.trim = "searchForm.orgCode"
            clearable
          ></el-input>
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        ></el-table-column>
        <el-table-column
          prop  = "managingStaffName"
          label = "专管员"
          width = "150"
          sortable
        ></el-table-column>
        <el-table-column
          prop  = "managingStaffPhone"
          label = "专管员电话"
          width = "150"
          sortable
        ></el-table-column>
      ...
      >
        ...
        >
          ...
          >
            <el-col :span="10">
              <el-input v-model="ruleForm.managingStaffPhone"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item
            label = "法人代表"
          ...
          >
            <el-col :span="10">
              <el-input v-model="ruleForm.legalRepresentativePhone"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item
            label = "状态"
      ...
      <!-- 详情模态框 -->
      <el-dialog title="护理机构详情" :visible.sync="dialogVisible2" width="60%" top="50px">
        <el-form
          :model="detailForm"
          :rules="rules"
          label-width="15%"
        ...
        >
          ...
          <el-form-item label="机构类型" width="200">
            <el-radio-group v-model="detailForm.orgType">
              <el-radio :label="4">机构护理类型</el-radio>
              <el-radio :label="5">居家护理类型</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item
            label = "机构性质"
          ...
          >
            <el-col :span="20">
              <el-input v-model="detailForm.managingStaffPhone"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item
            label = "法人代表"
          ...
          >
            <el-col :span="20">
              <el-input v-model="detailForm.legalRepresentativePhone"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item
            label = "状态"
...
export default {
  ...
  methods: {
    ...
    addData() {
      localStorage.setItem('nursingOrgAction',1);//0---修改；；；1-----新增；；；2-----查看
      localStorage.setItem("nursingOrgId", null);
      localStorage.setItem('nursingOrgSubmitBar',1);
      this.$router.push({ path: "nursing_orgs/nursing_org_detail" });
    },
    updateData() {
      if (this.checkData.length != 1) {
        ...
        localStorage.setItem('nursingOrgAction',0);//0---修改；；；1-----新增；；；2-----查看
        localStorage.setItem("nursingOrgId", data.orgId);
        this.$router.push({ path: "nursing_orgs/nursing_org_detail" });
      }
    },
    deleteData() {
      let _this = this;
      if (this.checkData.length != 1) {
        this.$message({ type: "info", message: "请选择一条数据进行操作" });
      } else {
    ...
    protocolDetails(index, rowData) {
      ...
      localStorage.setItem("nursingOrgId", data.orgId);
      localStorage.setItem("nursingOrgAction",2);//0---修改；；；1-----新增；；；2-----查看
          ...
          if (this.modelType) {
            ...
            this.$axios({
              ...
              if (res.data.data) {
                ...
                this.$message({ type: "info", message: "保存失败服务出错！" });
                this.dialogVisible = false;
              }
            });
          } else {
      ...
      that
        .$axios({
          url: `${that.$store.state.globalRouter}/auth/organization/${
            that.module
          }/pagination?pageFlag=45&pageNo=1&pageSize=${that.changePageSize}`,
          method: "get"
        })
        .then(res => {
          if(res.data.resultCode=='100005'){
      ...
      for (let i = 0; i < menu.length; i++) {
        if (menu[i].name == "机构管理") {
          ...
          for (let j = 0; j < submenu.length; j++) {
            if (submenu[j].component == "core/nursing_org") {
              // console.log("护理机构", submenu[j]);
              // urlRole1==dataAdd 增
