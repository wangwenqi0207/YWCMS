 ...
 <div style="padding:20px;height:100%">
    ...
    <div style="overflow:hidden;height:100px;width:96%;margin:20px auto 30px;text-align:center;color:#fff;">
      <div style="width:28%;background:#eee;height:100%;float:left;padding:10px;background:#008a8a;">
        ...
        <p>初评：<span>{{data1.firstAssOrderTotal}}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;复评：<span>{{data1.reAssOrderTotal}}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;持续评估：<span>{{data1.followAssOrderTotal}}</span></p>
      </div>
...
export default {
    data() {
        return {
            ...
            data1:{
                orderTotal:0,
                firstAssOrderTotal:0,
                reAssOrderTotal:0,
                followAssOrderTotal:0,
    ...
    methods: {
        ...
        this.$axios({
                        ...
                        }/auth/assess/assess_chart_amount/get_index`,
                        method: "get"
                      }).then(res => {
                        if (res.data) {
                          this.data1=res.data;
                ...
                methods: {
                    yearClick(year){
                      that.$axios({
                        ...
                        }/auth/assess/assess_chart_amount/get_chart_by_year?year=${year}`,
                        method: "get"
                      }).then(res => {
                        if (res.data) {
                          // 清空柱状图
                ...
                methods: {
                    monthClick(month){
                      that.$axios({
                        url: `${
                          that.$store.state.globalRouter
                        }/auth/assess/assess_chart_amount/get_chart_by_year_and_month?year=${$("#selectedYear").val()}&month=${month}`,
                        method: "get"
                      }).then(res => {
                        if (res.data) {
                          // 清空柱状图
