<template>
  <div style="padding:20px;height:100%;" id="nursingHome">
    <div style="background:#ffffff;height:95%;padding:5px;overflow:auto;">
      ...
      >
        ...
        <el-table-column prop="distributeOrgName" label="机构" width="220"></el-table-column>
        ...
        <el-table-column prop="comments" label="暂停原因" width="200">
          <template slot-scope="scope">
            <p v-if="scope.row.comments && scope.row.comments.length>10" style="display:inline-block;">
              {{scope.row.comments.substring(0,10)}}
            </p>
            <p v-else>
              {{scope.row.comments}}
            </p>
        ...
        <el-table-column prop="pauseDate" label="暂停时间" width="200"></el-table-column>
        <el-table-column prop="schedulingAuditStatus" label="审核状态" width="80"></el-table-column>
          ...
          <template slot-scope="scope">
            <div style="display:inline-block;">
              <el-button
                size="mini"
                type="success"
                @click.stop="planDetail(scope.$index, scope.row,'no')"
                style="margin:0px;"
              >护理计划详情</el-button>
              <el-button
                v-if="scope.row.schedulingAuditStatus!='审核通过' && urlRole4==1"
                size="mini"
                type="success"
                @click.stop="planDetail(scope.$index, scope.row,'yes')"
                style="margin:0px;"
              >审核</el-button>
            </div>
            <!-- <div
              v-if  = "scope.row.schedulingAuditStatus!='审核通过'"
        ...
        <div class="modal-body" id="hlxmModal">
          <form class="form-horizontal" style="overflow:hidden">
            ...
            <div class="form-group">
              ...
              <!--<div class="col-md-9">
                                            <select class="form-control input-sm" name="serviceStaff"></select>
              </div>-->
            </div>
      ...
      >
        ...
        <div class="modal-body">
          <form role="form" id="modalFrom" class="form-horizontal">
            ...
            <!-- <div class="h6_body" id="base_info" hidden='true'> -->
            <div class="h6_body" id="base_info" style="display:none">
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <div class="col-md-4 col-xs-4">
                  ...
                  <!-- <input
                    ...
                    name    = "serviceInDate"
                    type    = "text"
                    onfocus = "WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'planStartDate\')}'});"
                  />-->
                </div>
                  ...
                  <h6>
                    ...
                    <span class="pull-right">
                      <button
                        ...
                        @click="lastweek1()"
                      ></button>
                      <button name="this-week1" type="button" class="btn btn-sm" @click="thisweek1()">本周</button>
                      <button
                        name="next-week1"
                        type="button"
                  ...
                  <h6>
                    ...
                    <span class="pull-right">
                      <button
                        ...
                        @click="lastweek2()"
                      ></button>
                      <button name="this-week2" type="button" class="btn btn-sm" @click="thisweek2()">本周</button>
                      <button
                        name="next-week2"
                        type="button"
                        class="btn btn-sm glyphicon glyphicon-chevron-right"
...
export default {
  ...
  data() {
    return {
      ...
      pickerOptions2: {
        disabledDate(time){
          return time.getTime()  < window.localStorage.getItem('sjxz');
        }
		  },
      //分页data
      currentPage: 1,
  ...
  methods: {
    ...
    search() {
      this.$axios({
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/pagination?pageFlag=2&distributeOrgId=${
        ...
        }&schedulingAuditStatus=${encodeURI(this.shenheStatus)}&pageSize=${
          encodeURI(this.changePageSize)
        }&pageNo=1&comments=${encodeURI(this.stopReason)}`,
        method: "get"
      }).then(res => {
      ...
      if (this.hlsj == null || planId == "") {
        ...
        if (this.currentDate != "ALL") {
          ...
          if (this.serviceType == 1) {
            ...
            //  nursingServiceSchedulingPlan.serviceDateString = this.weekdayArray1;
            // console.log("1", nursingServiceSchedulingPlan);
            ...
            // nursingServiceSchedulingPlan.serviceDateString = this.weekdayArray2;
            // console.log("2", nursingServiceSchedulingPlan);
          } else {
            alert("护理类型错误");
          }
        }
    ...
    ajaxSubmitSchedulingPlan(nursingServiceSchedulingPlan, that) {
      ...
      that
        .$axios({
          ...
          }/auth/nursing/nursing_service_home_service_mng/submit_scheduling_plan`,
          method: "post",
          params: { nursingServiceSchedulingPlanJson: str }
        })
        .then(res => {
    ...
    ajaxCancleSchedulingPlan(nursingServiceSchedulingPlan, that) {
      ...
      that
        .$axios({
          // url   : `${that.$store.state.globalRouter}/auth/nursing/nursing_service_home_service_mng/cancel_scheduling_plan?nursingServiceSchedulingPlanJson=${str}`,
          // method: 'get',
          // 用params可以实现
          url: `${
            that.$store.state.globalRouter
          }/auth/nursing/nursing_service_home_service_mng/cancel_scheduling_plan`,
          method: "get",
          params: {
        ...
        this.$confirm("是否要删除当前数据?", "提示", {
          ...
          .then(() => {
            this.$axios({
              url: `${
                this.$store.state.globalRouter
              }/auth/nursing/nursing_service_home_service_mng/delete_scheduling?planId=${
    ...
    stop() {
      if (this.updateData2.length != 1) {
        ...
        this.planId=this.updateData2[0].planId;
        ...
        //       url: `${
        //         this.$store.state.globalRouter
        //       }/auth/nursing/nursing_service_plan_base_mng/pause_service?planId=${
        //         encodeURI(this.updateData2[0].planId)
        //       }&pauseStatus=2`,
        //       method: "get"
        //     }).then(res => {
    ...
    submitFormStop(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          // console.log(this.planId, this.ruleFormStop.stopWhy);
          this.$axios({
                url: `${
                  this.$store.state.globalRouter
                }/auth/nursing/nursing_service_plan_base_mng/pause_service?planId=${
                  encodeURI(this.planId)
                }&pauseStatus=2&comments=${encodeURI(this.ruleFormStop.stopWhy)}`,
                method: "get"
              }).then(res => {
                if ((res.data.resultCode == "000000")) {
                  this.$options.methods.getReloadData(this);
                  // this.$options.methods.closeModal('stopmodal');
                  // $('#stopmodal').hide();
    ...
    addHomeService() {
      ...
      // console.log(planId,this.startDate);
      // var serviceInDate = myModelTable
      ...
      if (this.curOpt_ == 1) {
        this.$axios({
          url: `${
            this.$store.state.globalRouter
          }/auth/nursing/nursing_service_home_service_mng/add_home_service?planId=${encodeURI(planId)}&serviceInDate=${encodeURI(serviceInDate)}`,
          method: "post"
        }).then(res => {
          if (res.data.resultCode == "000000") {
            this.$options.methods.getReloadData(this);
        ...
        this.$axios({
          url: `${
            this.$store.state.globalRouter
          }/auth/nursing/nursing_service_home_service_mng/add_home_service?planId=${encodeURI(planId)}&serviceInDate=${
            encodeURI(this.startDate)
          }`,
          method: "post"
        }).then(res => {
          if (res.data.resultCode == "000000") {
      ...
      //   url: `${
      //     this.$store.state.globalRouter
      //   }/auth/nursing/nursing_service_home_service_mng/add_home_service?planId=${planId}&serviceInDate=${
      //     this.startDate
      //   }`,
      //   method: "post"
      // }).then(res => {
    ...
    auditHomeService() {
      ...
      this.$axios({
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/audit_home_service?planId=${
    ...
    paibanDetail(planId, that) {
      ...
      that
        ...
        .then(res => {
          ...
          //   .find("input[name='serviceInDate']")
          //   .attr("disabled", true);
          $("#serviceInDate").attr("disabled", true);
          ...
          console.log('111',result.nursingServicePlanItemList)
          ...
          if (serviceType1Count != 0) {
            var MyComponent = Vue.extend({
              ...
              methods: {
                paiban(event) {
                  ...
                  console.log(event.srcElement.parentElement.childNodes[0].childNodes)
                  var ht = event.target;
                  var dateString = event.target.getAttribute("name");
                  // console.log(dateString.split("_")[1]);
                  ...
                  if (event.target.textContent != "+") {
                    ...
                    var hlryName = event.target.textContent.split(" ")[1];
                  }
                  console.log('66666',hlryName)
            ...
            var divData = schedulingTable1.find("tbody");
            divData.remove();
            schedulingTable1.append("<tr><td colspan='10' style='text-align: center;'>暂无数据</td></tr>");
          }
          if (serviceType2Count != 0) {
            ...
            schedulingTable2.append("<tr><td colspan='10' style='text-align: center;'>暂无数据</td></tr>");
          }
    ...
    planDetail(index, row, flag) {
      ...
      this.$axios({
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/load_ins_user_by_plan_id?planId=${encodeURI(planId)}`,
        method: "get"
      }).then(res => {
        var result = res.data.data;
        var myModelTable = $("#modalFrom");
        ...
        var nursingServicePlanItemList1 = new Array();
        var nursingServicePlanItemList2 = new Array();
        for (var i2 = 0; i2 < result.nursingServicePlanItemList.length; i2++) {
          var nursingServicePlanItem = result.nursingServicePlanItemList[i2];
          switch (nursingServicePlanItem.serviceType) {
            case 1:
              nursingServicePlanItemList1.push(nursingServicePlanItem);
    ...
    copyLastWeek1() {
      // console.log(this.updateData2[0].planId,this.weekdayArray1,this.servicePlanItemIdType1Array);

      this.$axios({
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/copy_last_week_scheduling_plan?planId=${
          encodeURI(this.updateData2[0].planId)
        }&servicePlanItemIdArray=${
    ...
    findInsUser(caseNo, that) {
      ...
      this.$axios({
        ...
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/load_ins_user_by_id_card_or_medical_no?idcardOrMedicalNo=${encodeURI(idcardOrMedicalNo)}&caseNo=${encodeURI(caseNo)}`,
        method: "get"
      }).then(res => {
        var result = res.data.data;
        ...
        if (result.idNo == null) {
          ...
          if(res.data.resultCode == '9999999' && res.data.resultMsg!='护理计划已被接收'){
             $("#userAge").focus();
            alert(
              "身份证号为：" +
                myModelTable.find("input[name='userIdcard']").val() +
        ...
        // }
          ...
          // }
            ...
            $("#userAge").focus();
            alert('该护理计划已被机构护理分配过床位，无法再次分配');
            this.$options.methods.clearModelTable();
          }
          else
          if (result.assessedLevel == null) {
            $("#userAge").focus();
            alert(
              "身份证号为：" +
                myModelTable.find("input[name='userIdcard']").val() +
          ...
          // }
            ...
            myModelTable
              ...
              .val(result.planStartDate);
            // 时间限制
            window.localStorage.setItem('sjxz',this.$moment(result.planStartDate, 'YYYY-MM-DD').valueOf())
            myModelTable.find("input[name='endDate']").val(result.endDate);
            ...
            $("#serviceInDate").val(result.serviceInDate);
            // this.serviceInDate=result.serviceInDate;
            $("#serviceInDate").attr("disabled", false);
            myModelTable.find("input[name='endInDate']").val(result.endDate);
            myModelTable.find("input[name='pauseDate']").val(result.pauseDate);
            ...
            for (var i = 0; i < result.nursingServicePlanItemList.length; i++) {
              ...
              if (nursingServicePlanItem.serviceType == "1") {
                tableHTML +=
                  "<td style='text-align:center;vertical-align: middle;'>基础护理</td>";
                serviceType1Count++;
                this.servicePlanItemIdType1Array.push(
                  nursingServicePlanItem.id
                );
              } else if (nursingServicePlanItem.serviceType == "2") {
                tableHTML +=
                  "<td style='text-align:center;vertical-align: middle;'>常用临床护理</td>";
                serviceType2Count++;
                this.servicePlanItemIdType2Array.push(
                  nursingServicePlanItem.id
                );
              }
              tableHTML +=
                  ...
                  paiban(event) {
                    ...
                    if (data.serviceType == 1) {
                      ...
                      if (hlryName != null) {
                        for (
                          var i = 0;
                          i < that.serviceStaffData2.length;
                          i++
                        ) {
                          if (that.serviceStaffData2[i].fullname == hlryName) {
                            that.serviceCurrentStaff =
                    ...
                    // that.serviceStaff = that.serviceStaffData[0].userId;
                  }
                }
              });
              var component = new MyComponent().$mount();
              ...
              var MyComponent2 = Vue.extend({
                ...
                methods: {
                  paiban(event) {
                    ...
                    // that.serviceStaff = that.serviceStaffData2[0].userId;
                  }
                }
              });
              var component2 = new MyComponent2().$mount();
    ...
    handleCurrentChange(val) {
      this.$axios({
        url: `${
          this.$store.state.globalRouter
        }/auth/nursing/nursing_service_home_service_mng/pagination?pageFlag=2&pageSize=${
          encodeURI(this.changePageSize)
        }&pageNo=${val}&distributeOrgId=${encodeURI(this.disOrg1)}&userNameLike=${
          encodeURI(this.nameSearch)
        }&medicalInsuranceIdLike=${encodeURI(this.ybCardSearch)}&userIdcardLike=${
          encodeURI(this.idCardSearch)
        }&planStatus=${encodeURI(this.status)}&schedulingAuditStatus=${encodeURI(this.shenheStatus)}&comments=${encodeURI(this.stopReason)}`,
        method: "get"
      }).then(res => {
        this.total = res.data.totalCount;
    ...
    getReloadData(that) {
      that
        .$axios({
          ...
          }&pageNo=${encodeURI(that.currentPage)}&distributeOrgId=${
            encodeURI(that.disOrg1)
          }&userNameLike=${encodeURI(that.nameSearch)}&medicalInsuranceIdLike=${
            encodeURI(that.ybCardSearch)
          }&userIdcardLike=${encodeURI(that.idCardSearch)}&planStatus=${
            encodeURI(that.status)
          }&schedulingAuditStatus=${encodeURI(that.shenheStatus)}&comments=${encodeURI(that.stopReason)}`,
          method: "get"
        })
        .then(res => {
  ...
  mounted() {
    // 时间限制
    window.localStorage.setItem('sjxz',this.$moment('2019-05-03', 'YYYY-MM-DD').valueOf())
    // this.getData();
    this.$axios({
      url: `${
        this.$store.state.globalRouter
      }/auth/nursing/nursing_service_home_service_mng/pagination?pageFlag=2`,
      method: "get"
    }).then(res => {
      // this.total       = res.data.totalCount;
      // this.currentPage = res.data.curPage;
    ...
    this.$axios({
      url: `${
        this.$store.state.globalRouter
      }/auth/intact/intact_organization_mng/load_by_org_type?orgType=5`,
      method: "get"
    }).then(res => {
    ...
    this.$axios({
      ...
      // 后台限制返回1000条数据
      // for(var i=0;i<this.serviceStaffData.length;i++){
      //   if(this.serviceStaffData[i].fullname=='719朱苏琴'){
      //     console.log('成功')
      //   }
      // }
      // console.log('护理人员：',this.serviceStaffData)
    });
    // 护士选择
    this.$axios({
      url: `${
      ...
      // console.log('护士：',this.serviceStaffData2)
    });
      ...
      for (let i = 0; i < menu.length; i++) {
        if (menu[i].name == "护理管理") {
          ...
          for (let j = 0; j < submenu.length; j++) {
            if (submenu[j].component == "nursing/nursing_home") {
              console.log("居家护理", submenu[j]);
              // urlRole1==dataAdd 增
...
<style>
#nursingHome .el-dialog__header{
margin-bottom:5px!important;
}
#nursingHome .el-dialog__body{
padding-top:0px;
padding-bottom: 0px;
}
</style>
